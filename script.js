...
document.addEventListener('DOMContentLoaded', function () {
        ...
        <div class="tour-detail-container">
          ...
          <div class="guide-section-box">
            ...
            <ul style="margin-left:20px; line-height:1.8;">
              ...
              <li><strong>Chinnar Wildlife Sanctuary:</strong> Border jungle stretch where spotter deer, wild elephants, and giant squirrels are frequently spotted.</li>
  ...
  // 12. Intelligent AI Chatbot Engine
  const aiChatHistory = [];
  ...
  if (chatSendBtn) chatSendBtn.addEventListener('click', window.handleAiSend);
  if (chatInputEl) {
  ...
  function generateAiResponse(input) {
    ...
    if (query.includes('ooty') || query.includes('nilgiri') || query.includes('coonoor')) {
      return `<strong>🚕 Coimbatore to Ooty / Coonoor Cab Details:</strong><br>
    ...
    if (query.includes('airport') || query.includes('cjb') || query.includes('flight')) {
      return `<strong>✈️ Coimbatore Airport (CJB) Fixed Drop Rates:</strong><br>
    ...
    if (query.includes('contact') || query.includes('phone') || query.includes('call') || query.includes('book')) {
      return `<strong>📞 Contact Get Cabs Coimbatore 24/7:</strong><br>
    ...
    return `I am Get Cabs AI Assistant! I can tell you about:<br>
      • <strong>Local City Taxi Services</strong><br>
      • <strong>Fixed Oneway Drop Rates (Ooty ₹3,500, Pollachi ₹1,600)</strong><br>
      • <strong>Day Rental Packages</strong><br>
  ...
  if (spinWheelBtn) {
    spinWheelBtn.addEventListener('click', function() {
      if (hasSpun) {
        if (wheelResultMsg) wheelResultMsg.innerHTML = '🎁 You already won! Use Coupon Code <strong style="color:var(--brand-red);">GET100</strong> for ₹100 Off.';
        return;
      }
      const randomDegrees = 1440 + Math.floor(Math.random() * 360);
      if (wheelGraphic) wheelGraphic.style.transform = `rotate(${randomDegrees}deg)`;
...
window.initPlacesAutocomplete = function() {
  ...
  // Automatic Distance Matrix calculation
